...
const plan = await run(process.execPath, [installer, ...base, '--json']); if (plan.code !== 0 || JSON.parse(plan.stdout).status !== 'plan_only') throw new Error(`Hermes install plan failed: ${plan.stderr}`);
